...
def votingPg(root, frame1, client_socket):
    ...
    Radiobutton(frame1, text="JDS\n\nH.D.Kumarswamy", variable=vote, value="ss", indicator=0, height=4, width=15, command=lambda: voteCast(root, frame1, "ss", client_socket)).grid(row=5, column=1)
    ssLogo = ImageTk.PhotoImage((Image.open("img/Jds.png")).resize((50, 45), Image.Resampling.LANCZOS))
...
# Uncomment this block to run the script standalone
# if _name_ == "_main_":
#     root = Tk()
#     root.geometry('500x500')
#     frame1 = Frame(root)
#     client_socket = 'Fail'  # Replace with an actual socket object if you have one
